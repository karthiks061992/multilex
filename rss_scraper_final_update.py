...
# using now() to get current time  
current_time = datetime.datetime.now()            
tod_date=str(current_time.day-1)
tod_date_post=int(tod_date)+1
tod_date_post=str(tod_date_post)
if len(tod_date)==1:
  tod_date='0'+tod_date
...
for i in range(0,df1.shape[0]):
  ...
  rep_date=rep_date.split(' ')
  rep_date=rep_date[1]
  if rep_date==tod_date or rep_date==tod_date_post:
...
scraped_date=[]
flag=False
keywords=['IPO','IPO','IPO ','IPOs','SPACs','ipo','pre-IPO','pre-ipo','PRE-IPO','pre-IPO','going public','public','closes','listing','planning','closing','excellent','Public','Initial','Offering','initial','Announces','Pricing','pricing','announces','launches','Launches','SPAC','spac']
for i in range(0,df1.shape[0]):
  article=df1['Text'][i]
...
for i in range(0,df_google.shape[0]):
  ...
  if combined_date==tod_date or combined_date==tod_date_post:
...
df_final.to_csv('rss_10th-11thjan2021.csv')
!cp rss_9thjan2021.csv "drive/My Drive/"